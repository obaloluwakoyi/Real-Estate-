...
def preprocess_data(
    ...
    # ── 3. Scale — FIX: fit scaler ONLY on training data to prevent data leakage
    train_data = dataset[:training_data_len]
    scaler = StandardScaler()
    scaler.fit(train_data)                             # fit on train only
    scaled_data = scaler.transform(dataset)            # transform full dataset
    logging.info(f"Scaler fitted on train only  μ={scaler.mean_[0]:.4f}  σ={scaler.scale_[0]:.4f}")
